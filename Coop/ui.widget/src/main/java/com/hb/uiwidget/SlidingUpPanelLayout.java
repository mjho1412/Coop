...
public class SlidingUpPanelLayout extends ViewGroup {
    ...
    private static final int NO_FADE_COLOR = Color.TRANSPARENT;
    /**
    ...
    private boolean mNoCollapse = false;
//    private int mScrollableViewTopPadding;

    private int mOverExtend = 0;
    ...
    private int mOriginalHeight, mFirstChildHeight;
    private boolean mfadeChild = true;
    ...
    public SlidingUpPanelLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        setCoveredFadeColor(NO_FADE_COLOR);
    ...
    public void setPanelHeight(int val) {
        mPanelHeight = val + mOverExtend;
        requestLayout();
    }

    public void setOverExtend(int h) {
        mPanelHeight -= mOverExtend;
        mOverExtend = h;
        mPanelHeight += h;
        requestLayout();
    }
    ...
    private void customslide() {
        if (mResize) {
            if (mSlideOffset > mAnchorPoint)
                mFirstChildHeight = (int) (mOriginalHeight * mSlideOffset);
            this.requestLayout();
        }
    }
    ...
    void dispatchOnPanelSlide(View panel) {
        ...
        if (mImageView != null) {
            if (!(mScrollableView instanceof ScrollView) || mSlideOffset == 1 || mSlideOffset <= 0)
                mImageView.setVisibility(View.GONE);
            else {
                mImageView.setVisibility(View.VISIBLE);
                RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) mImageView.getLayoutParams();
                params.height = (int) (mAnchorPoint * mSlideRange);
                mImageView.setLayoutParams(params);
                if (mSlideableView != null) {
                    mImageView.setY(mSlideableView.getY() - params.height);
                }
                float alpha = (1f - mSlideOffset) / (1f - mAnchorPoint);
                mImageView.setAlpha(alpha > 1 ? 1 : alpha);
            }
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (i == 1) {
                ...
                if (mFirstChildHeight != 0)
                    height = mFirstChildHeight;
            }
            height += mOverExtend;
            int childWidthSpec;
    ...
    public void overlayBackground(boolean overlay) {
        mOverlay = overlay;
    }
...
//                if (!mDragViewHit && isAnchored() && !(mScrollableView instanceof RecyclerView))
                if (!mDragViewHit && isAnchored())
                    return true;
...
//                		(mScrollableView != null && mScrollableView.getScrollY() != 0));
//        		if (isCanScrollUp || (ady > dragSlop && adx > ady)) {
//            		mDragHelper.cancel();
//            		mIsUnableToDrag = true;
//            		return false;
//        		}
//                Log.e("TEST", mScrollableView.getTop() + " " + mInitialMotionY);

                if (mScrollableView instanceof ListView) {
                    ListView lv = (ListView) mScrollableView;
                    boolean isCanScrollUp = mSlideOffset == 0 && (y < mInitialMotionY ||
                            lv.getFirstVisiblePosition() != 0 ||
                            (lv != null && lv.getChildAt(0) != null && lv.getChildAt(0).getTop() != lv.getPaddingTop()));
                ...
                if (mScrollableView instanceof RecyclerView) {
                    RecyclerView rcV = (RecyclerView) mScrollableView;

                    boolean isCanScrollUp = mSlideOffset == 0 && (y < mInitialMotionY ||
                            ((LinearLayoutManager) rcV.getLayoutManager()).findFirstCompletelyVisibleItemPosition() != 0);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (isExpanded()) {
                    ...
                    if (mScrollableView instanceof ScrollView) {
                        ...
                        if (y < lastY || sv.getScrollY() != 0) {
                            if (lastY != 0) {
                                sv.scrollBy(0, (int) (lastY - y));
                                redrag = true;
                            }
                        } else {
                            if (redrag) {
...
//	                		if (lastY != 0) {
//	                			float delY = y - lastY;
//	                			float max = (mAnchorPoint - mSlideOffset) * mSlideRange;
//	                			if (delY > max) {
////	                				mDragHelper.smoothSlideViewTo(mSlideableView, 0, (int) (mAnchorPoint * mSlideRange));
////	                				mDragHelper.cancel();
//	                				lastY = y;
//	                				return false;
//	                			}
...
//                	}
    ...
    public void enableResize(boolean bvalue) {
        mResize = bvalue;
    }
    ...
    public boolean isOverExpand() {
        return mSlideOffset < 0 || (mOverExtend == 0 && isExpanded());
    }
...
//    private boolean isAnchoredAndBelow() {
//    	int anchoredTop = (int)(mAnchorPoint*mSlideRange);
//        return !mFirstLayout && mCanSlide
//                && mSlideOffset >= (float)anchoredTop/(float)mSlideRange;
//    }
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (yvel > 0 && mScrollableView instanceof ScrollView && mScrollableView.getScrollY() != 0)
                canScroll = true;
            if (isOverExpand() && yvel != 0 && mScrollableView instanceof ScrollView)
                ((ScrollView) mScrollableView).fling((int) -yvel);
            if (mSlideOffset >= 0) {
                if (mAnchorPoint != 0) {
                    int anchoredTop = (int) (mAnchorPoint * mSlideRange);
                    float anchorOffset = (float) anchoredTop / (float) mSlideRange;

                    if ((yvel > 0 && !canScroll) || (yvel == 0 && mSlideOffset >= (1f + anchorOffset) / 2)) {
                        if (mNoCollapse)
                ...
                } else if ((yvel > 0 && !canScroll) || (yvel == 0 && mSlideOffset > 0.5f)) {
                    if (mNoCollapse)
                        top += mSlideRange * mAnchorPoint;
                    else
                        top += mSlideRange;
        ...
        public int getViewVerticalDragRange(View child) {
            return mSlideRange + mOverExtend;
        }

        @Override
        public int clampViewPositionVertical(View child, int top, int dy) {
            final int topBound = getPaddingTop() - mOverExtend;
            final int bottomBound;
            if (mNoCollapse)
                bottomBound = (int) (topBound + mSlideRange * mAnchorPoint + mOverExtend);
            else
                bottomBound = topBound + mSlideRange + mOverExtend;
    ...
    private boolean isDragViewUnder(int x, int y) {
        ...
        return screenX >= viewLocation[0] && screenX < viewLocation[0] + dragView.getWidth() &&
                (screenY >= viewLocation[1] + mScrollableView.getPaddingTop()) && screenY < viewLocation[1] + dragView.getHeight();
    }

    public void setFadeChild(boolean b) {
        mfadeChild = b;
    }
...
//		mSlideState = SlideState.EXPANDED;
    ...
    public void setNoCollapse(boolean bvalue) {
        mNoCollapse = bvalue;
    }

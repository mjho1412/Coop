...
public class MockLocationEngine extends LocationEngine {
    ...
    private FindShortestPath route;
     ...
     *
     * @param route a {@link FindShortestPath} which you'd like to mock the user location on.
     * @since 2.2.0
     */
    public void setRoute(FindShortestPath route) {
        this.route = route;
...
//                route.legs().get(currentLeg).steps().get(currentStep).geometry(), Constants.PRECISION_6);
        FindShortestPath.Step step = route.getResultScript().getLegs()[currentStep].getSteps()[currentStep];

        LineString line = LineString.fromLngLats(Arrays.asList(
                Point.fromLngLat(step.getX(), step.getY()),
                Point.fromLngLat(step.getX(), step.getY())
        ));
    ...
    private void increaseIndex() {
        ...
        if (currentStep < route.getResultScript().getLegs()[currentLeg].getSteps().length - 1) {
            currentStep++;
        } else if (currentLeg < route.getResultScript().getLegs().length - 1) {
            currentLeg++;
            currentStep = 0;
        }
    }
    ...
    /**
     * Here we show the new mock {@link Location} object and fill in as much information we can calculate.
     *

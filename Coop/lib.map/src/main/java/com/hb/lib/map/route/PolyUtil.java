...
import static com.hb.lib.map.route.MathUtil.mercator;
import static com.hb.lib.map.route.MathUtil.wrap;
import static com.hb.lib.map.route.SphericalUtil.computeDistanceBetween;
...
public class PolyUtil {
    ...
    static public int snapIndex(LatLng ll, List<LatLng> path, double tolorence) {
        double minDist = Double.MAX_VALUE;
        int snapIndex = -1;
        int size = path.size();
        for (int i = 0; i < size - 1; i++) {
            double dist = distanceToLine(ll, path.get(i), path.get(i + 1));
            if (dist < tolorence && dist < minDist) {
                minDist = dist;
                snapIndex = i;
            }
        }
    ...
    public static int locationIndexOnPath(LatLng point, List<LatLng> poly,
        ...
        double y3 = mercator(lat3);
        double[] xTry = new double[3];
        double minHavDist = Double.MAX_VALUE;
        int rIdx = -1;
        for (LatLng point2 : poly) {
            double lat2 = toRadians(point2.getLatitude());
            double y2 = mercator(lat2);
            ...
            if (max(lat1, lat2) >= minAcceptable && min(lat1, lat2) <= maxAcceptable) {
                ...
                for (double x3 : xTry) {
                    ...
                    double havDist = MathUtil.havDistance(lat3, latClosest, x3 - xClosest);
                    if (havDist < havTolerance) {
                        if (havDist < minHavDist) {
                            rIdx = Math.max(0, idx - 1);
                        }
//                        return Math.max(0, idx - 1);
                    }
                }
